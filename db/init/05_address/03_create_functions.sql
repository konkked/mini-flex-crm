...
CREATE OR REPLACE FUNCTION entityAddress_set_significance_ordinal()
RETURNS TRIGGER AS $$
BEGIN
    NEW.significance_ordinal := COALESCE(
        (SELECT MAX(significance_ordinal) + 1 FROM entity_address WHERE entity_name = NEW.entity_name AND entity_id = NEW.entity_id),
        1
    );
    RETURN NEW;
END;
...
CREATE OR REPLACE FUNCTION entityAddress_adjust_significance_ordinal()
RETURNS TRIGGER AS $$
BEGIN
