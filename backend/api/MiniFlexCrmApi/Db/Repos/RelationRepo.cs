...
public class RelationRepo : DbEntityRepo<RelationDbModel>, IRelationRepo
{
    ...
    public override async IAsyncEnumerable<RelationDbModel> GetNext(int id, int count)
    {
    ...
    public override async IAsyncEnumerable<RelationDbModel> GetPrevious(int id, int count)
    {
        var results = await ConnectionProvider.Connection.QueryAsync<RelationDbModel>(
            @$"SELECT t1.*, t2.name as customer_name
                   FROM {TableName} t1
