...
public class SalesOpportunityController(ISalesOpportunityService service) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(int tenantId, [FromBody] SalesOpportunityModel model)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int tenantId, int id, [FromBody] SalesOpportunityModel model)
    {
        model.Id = id;
        model.TenantId = tenantId;
