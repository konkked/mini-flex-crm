...
[Route("api/tenant/{tenantId}/payment")]
[ApiController]
public class PaymentController(IPaymentService service) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(int tenantId, [FromBody] PaymentModel model)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int tenantId, int id, [FromBody] PaymentModel model)
    {
        model.Id = id;
        model.TenantId = tenantId;
