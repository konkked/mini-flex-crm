...
[Route("api/tenant/{tenantId}/interaction")]
[ApiController]
public class InteractionController(IInteractionService service) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(int tenantId, [FromBody] InteractionModel model)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int tenantId, int id, [FromBody] InteractionModel model)
    {
        model.Id = id;
        model.TenantId = tenantId;
