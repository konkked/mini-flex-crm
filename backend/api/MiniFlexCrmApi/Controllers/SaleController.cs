...
[Route("api/tenant/{tenantId}/sale")]
[ApiController]
public class SaleController(ISaleService service) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(int tenantId, [FromBody] SaleModel model)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int tenantId, int id, [FromBody] SaleModel model)
    {
        model.Id = id;
        model.TenantId = tenantId;
