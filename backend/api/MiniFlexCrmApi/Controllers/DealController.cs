...
[ApiController]
public class DealController(IDealService service) : ControllerBase
{
    [HttpGet("{id}")]
    ...
    [HttpPost]
    public async Task<IActionResult> Create(int tenantId, [FromBody] DealModel model)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int tenantId, int id, [FromBody] DealModel model)
    {
        model.Id = id;
        model.TenantId = tenantId;
