...
[Route("api/tenant/{tenantId}/support-ticket")]
[ApiController]
public class SupportTicketController(ISupportTicketService service) : ControllerBase
{
    [HttpGet("{id}")]
    ...
    [HttpPost]
    public async Task<IActionResult> Create(int tenantId, [FromBody] SupportTicketModel model)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int tenantId, int id, [FromBody] SupportTicketModel model)
    {
        model.Id = id;
        model.TenantId = tenantId;
