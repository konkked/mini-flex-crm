...
[Route("api/tenant/{tenantId}/address")]
[ApiController]
public class AddressController(IAddressService service) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(int tenantId, [FromBody] AddressModel model)
    {
        model.TenantId = tenantId;
        var id = await service.UpsertAsync(model);
        return CreatedAtAction(nameof(Get), new { tenantId, id }, model);
    }

    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int tenantId, int id, [FromBody] AddressModel model)
    {
        model.Id = id;
        model.TenantId = tenantId;
